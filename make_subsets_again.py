...
fraction_list = [0.05, 0.10, 0.25, 0.3, 0.4, 0.50, 0.6, 0.75,0.8,0.9,0.95,0.99]
...
for dataset in image_dataset_list:
    ...
    for baseline in ["image_based"]:
        for task in tasks:
            ...
            save_folder = f"experiments_again/{dataset}/{baseline}/"
            ...
            if not os.path.exists(save_folder+f"test{task[4]}_C=0.75_metrics.json") and os.path.exists(save_path):
...
for dataset in image_dataset_list:
    ...
    for baseline in baselines_list:
         ...
         if not os.path.exists(save_folder+"test1_C=0.75_metrics.json") and os.path.exists(save_path):
             subprocess.call(shlex.split('sbatch baselines/run_clip_linear_probe.sh "%s" "%s" "%s"'%(dataset, save_path, save_folder)))
